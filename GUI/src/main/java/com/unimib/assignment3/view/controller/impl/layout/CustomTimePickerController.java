...
import static com.unimib.assignment3.model.enums.TimeFormat.AMPM;
...
public class CustomTimePickerController implements DefaultController {
    ...
    private void initListeners() {
        ...
        timeFormat.addListener((_, _, _) -> updateUI());

        hourField.focusedProperty().addListener((_, _, newVal) -> {
            if (!newVal) updateCurrentTimeFromFields();
        });

        minuteField.focusedProperty().addListener((_, _, newVal) -> {
            if (!newVal) updateCurrentTimeFromFields();
        });
    }
